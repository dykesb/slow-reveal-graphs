...
emblems_import <- function(import_list) {
  ...
  for (i in 1:length(import_list)) {
    ...
    import <- readPNG(here::here("Slow Reveal Recreations", "gannett-most-populous-cities-12th-census", "city-emblems", str_c(as.character(import_list[i]), ".png")))
...
# Modifying Labels
data <- data |> 
  mutate(City = case_when(City == "lexington" ~ "lexington (ky.)",
                          TRUE ~ City),
...
png(here::here("Slow Reveal Recreations", "gannett-most-populous-cities-12th-census", "recreated-imgs", "01_full_unmasked.png"),width=9468,height=6262)
...
data |>
  ...
  geom_text(aes(x = -Census,
            ...
            data = data |>
              filter(label == 1) |> 
              mutate(City = str_replace(City, "_\\d$", ""))) +
  ...
  geom_text(x = -13,
            y = 2.25,
            label = "RANK OF THE MOST POPULOUS CITIES AT EACH CENSUS",
            size = 88/.pt) +
  geom_text(x = -1,
            y = 2.5,
            label = "PLATE No. 22",
